...
const COURSES_PUBLIC_ENDPOINT = `${API_BASE_URL}/api/v1/courses/landing`;
const AUTH_STORAGE_KEY = "lmsbasicg_auth";

const courseEndpoint = (courseId: number) => `${COURSES_ENDPOINT}/${courseId}`;
...
function resolveBusinessDomain(payloadDomain?: string): string {
  const configuredDomain = normalizeDomain(
  ...
  const domain =
    normalizeDomain(payloadDomain) || configuredDomain || browserDomain;

  if (!domain) {
    throw new Error("No se pudo identificar el dominio de la empresa.");
  }
...
async function parseErrorResponse(response: Response): Promise<never> {
  ...
  try {
    ...
    if (Array.isArray(parsed?.detail) && parsed.detail.length > 0) {
      ...
      throw new ApiError(normalizeCourseErrorMessage(message), response.status);
    }
    ...
    throw new ApiError(normalizeCourseErrorMessage(rawText), response.status);
  } catch (error) {
...
export async function getPublicCourses(
  ...
  const domain = resolveBusinessDomain(customDomain);
  const endpoint = `${COURSES_PUBLIC_ENDPOINT}/${domain}`;
