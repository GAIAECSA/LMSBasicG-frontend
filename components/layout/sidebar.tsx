...
function isDashboardHomePath(
    pathname: string,
) {
    return DASHBOARD_HOME_PATHS.includes(
        normalizePath(
            pathname,
        ),
    );
 ...
 *
 * Ejemplo:
 * /admin/courses/subcategories coincide con:
 * - /admin/courses
 * - /admin/courses/subcategories
 *
 * Se elige la segunda opción por ser más específica.
 */
...
function filterSidebarItemsByBusinessPlan(
    ...
    return items.flatMap(
        (
            ...
            /*
             * Si un grupo queda sin opciones visibles,
             * también se elimina del sidebar.
             */
            if (
                hadChildren &&
                !hasVisibleChildren &&
                            ...
                            >
                                ...
                                {!collapsed ? (
                                    <>
                                        ...
                                        <ChevronDown
                                            className={`h-4 w-4 shrink-0 transition-transform duration-150 ${isOpen
                                                ? "rotate-180"
                                                : ""
                                                }`}
                                        />
...
function AthenaSidebarLogo({
    ...
    return (
        <div
            className={`flex min-w-0 items-center ${collapsed
                ? "justify-center"
                : "gap-3"
                }`}
        >
...
export function Sidebar({
    ...
    /*
     * null significa que la configuración todavía no se ha cargado.
     *
     * Mientras sea null se mantiene el sidebar original para evitar
     * ocultar opciones por un fallo de red o por una sesión expirada.
     */
    /*
     * null significa que los planes todavía no se han cargado.
     *
     * Mientras el endpoint responde, todos los módulos normales
     * permanecen visibles y las opciones MDT permanecen ocultas.
     */
    const [
        businessPlans,
        setBusinessPlans,
    ...
    useEffect(() => {
        ...
        async function loadBusinessPlans() {
            try {
                ...
                console.info(
                    "Planes habilitados para la empresa:",
                    plans,
                );
            } catch (
            error
                ...
                /*
                 * Si ocurre un error, se muestran todos los módulos
                 * normales y MDT permanece oculto.
                 */
                console.warn(
