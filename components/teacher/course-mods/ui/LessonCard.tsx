...
/*
 * En la pantalla de módulos deben mostrarse todos los bloques
 * normales, tanto activos como inactivos.
 *
 * El campo default ya no se utiliza como filtro porque todos
 * los bloques normales se guardan con default = false.
 *
 * Únicamente se mantienen ocultos los archivos obligatorios MDT,
 * ya que estos se administran desde su propia pantalla.
 */
...
export function LessonCard({
    ...
    const lessonIsDragging =
        mods.isDraggingItem(lessonDragState);

    const lessonIsOver =
        mods.isDragOverItem(lessonDragState);

    const visibleItems = lesson.items.filter(
        shouldShowItemInModuleList,
    );

    return (
        <div
            draggable
            className={`min-w-0 cursor-grab rounded-xl border bg-white p-3 transition active:cursor-grabbing sm:rounded-2xl sm:p-4 ${lessonIsDragging
                    ? "opacity-50"
                    : ""
                } ${lessonIsOver
            ...
            <div className="flex min-w-0 flex-col gap-3">
                <div className="flex min-w-0 items-center justify-between gap-2">
                    ...
                    >
                        ...
                        <ChevronDown
                            className={`h-4 w-4 shrink-0 text-slate-500 transition sm:h-5 sm:w-5 ${lessonOpen
                                    ? ""
                                    : "-rotate-90"
                                }`}
                        />
