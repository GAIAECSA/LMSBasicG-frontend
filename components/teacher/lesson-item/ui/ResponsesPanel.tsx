...
export function ResponsesPanel({ item, block }: ResponsesPanelProps) {
    ...
    const selectedResponseText = selectedResponse
        ? getActivityResponseText(selectedResponse)
        : "";

    const selectedResponseStudent = selectedResponse
        ? getActivityResponseStudent(selectedResponse)
        : "";

    const selectedResponseDate = selectedResponse
        ? formatDate(selectedResponse.updated_at ?? selectedResponse.created_at)
        : "";
    ...
    return (
        <>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6 [@media(max-height:760px)]:p-4">
                <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                        type="button"
                        onClick={() =>
                            void item.handleRefreshActivityResponses(block)
                        }
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-[10px] font-semibold text-slate-500 sm:text-xs">
                                            Matrícula #{response.enrollment?.id ??
                                ...
                                <div className="mt-3 flex items-center justify-between gap-2">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex h-8 items-center justify-center gap-1 rounded-xl bg-slate-100 px-3 text-[11px] font-black text-slate-700 transition hover:bg-slate-200 active:scale-[0.97]"
                                    >
            ...
            <ResponseModal
                ...
                student={selectedResponseStudent}
                date={selectedResponseDate}
