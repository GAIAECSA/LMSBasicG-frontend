...
import { formatMoney, parseNumberInput } from "../utils";
...
type CourseFormModalProps = {
    ...
    onUpdateForm: <K extends keyof CourseFormState>(
        key: K,
...
const inputClass =
    "h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 sm:h-11 sm:rounded-2xl sm:px-4 sm:text-sm";

const labelClass =
    "mb-1.5 block text-xs font-bold text-slate-700 sm:mb-2 sm:text-sm";
    ...
    return (
        ...
        >
            ...
            >
                <div className="shrink-0 bg-gradient-to-br from-[#07111F] via-[#172861] to-[#F97316] px-4 py-4 text-white sm:px-6 sm:py-5 [@media(max-height:760px)]:py-3">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 hidden text-xs text-blue-50 sm:block sm:text-sm">
                ...
                >
                    <div className="grid min-h-0 flex-1 overflow-y-auto lg:grid-cols-[minmax(0,1.3fr)_minmax(310px,0.7fr)] lg:overflow-hidden">
                            ...
                            <div className="space-y-4 [@media(max-height:760px)]:space-y-3">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none transition file:mr-2 file:rounded-lg file:border-0 file:bg-[#172861] file:px-3 file:py-1.5 file:text-xs file:font-bold file:text-white hover:file:bg-[#0B163F] sm:rounded-2xl sm:px-4 sm:py-2.5 sm:text-sm sm:file:rounded-xl sm:file:px-4 sm:file:py-2 sm:file:text-sm"
                                    />

                                    {selectedImageFile ? (
                                        <p className="mt-1.5 text-[11px] font-semibold text-slate-600 sm:text-xs">
                                            Archivo: {selectedImageFile.name}
                                        </p>
                                    ) : form.image_url ? (
                                        <p className="mt-1.5 text-[11px] font-semibold text-slate-500 sm:text-xs">
                                            Se mantiene la imagen actual si no
                                            seleccionas otra.
                                        </p>
                                    ) : null}
                                </div>
                    ...
                    <div className="grid shrink-0 grid-cols-1 gap-2 border-t border-slate-200 bg-white px-4 py-3 sm:grid-cols-2 sm:px-6">
                        <button
                            type="button"
                            onClick={onClose}
                            className="h-10 rounded-xl border border-slate-200 px-5 text-xs font-bold text-slate-700 transition hover:bg-slate-50 active:scale-[0.97] sm:h-11 sm:rounded-2xl sm:text-sm"
                        >
                            Cancelar
                        </button>
                        ...
                        <button
                            type="submit"
                            disabled={isSaving}
                            className="h-10 rounded-xl bg-[#172861] px-5 text-xs font-bold text-white transition hover:bg-[#0B163F] active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-70 sm:h-11 sm:rounded-2xl sm:text-sm"
                        >
                            {isSaving
                                ? "Guardando..."
                                : editingCourseId
    ...
    return (
        <div>
            ...
            <input
                ...
                className={`${inputClass} disabled:bg-slate-100 ${className}`}
            />
        </div>
    );
}
        ...
        <aside className="border-t border-slate-200 bg-gradient-to-b from-slate-50 via-white to-slate-100/70 px-4 py-4 sm:px-5 sm:py-5 lg:min-h-0 lg:overflow-y-auto lg:border-t-0 [@media(max-height:760px)]:py-3">
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:rounded-3xl">
                <div className="relative h-52 overflow-hidden bg-slate-100 sm:h-64 lg:h-56 xl:h-64 [@media(max-height:760px)]:lg:h-48">
                    ...
                    <div className="absolute inset-x-0 top-3 flex items-start justify-between gap-2 px-3">
                        <div className="flex flex-wrap gap-1.5">
                            <PreviewBadge>{form.level}</PreviewBadge>
                        ...
                        <div className="shrink-0 rounded-xl bg-white/95 px-2.5 py-1.5 text-xs font-bold text-slate-900 shadow-lg">
                            {form.is_free
                                ? "Gratis"
                                : formatMoney(
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4">
                        <h4 className="line-clamp-2 text-xl font-extrabold leading-tight text-white drop-shadow-sm sm:text-2xl">
                ...
                <div className="space-y-3 p-4">
                    <p className="line-clamp-3 text-xs leading-5 text-slate-600 sm:text-sm">
                        {form.description ||
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <PreviewValue
                            label="Matrícula"
                            value={
                                form.open_enrollment ? "Abierta" : "Cerrada"
                            }
                        />
                    </div>
                </div>
...
function PreviewBadge({
    ...
    return (
        <span
            className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide shadow-sm ${className}`}
        >
            {children}
        </span>
    );
