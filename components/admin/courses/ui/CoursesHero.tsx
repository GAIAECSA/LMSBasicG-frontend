...
export function CoursesHero({
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#07111F] via-[#172861] via-70% to-[#F97316] p-4 text-white shadow-lg sm:rounded-3xl sm:p-5 lg:p-6 [@media(max-height:760px)]:p-4">
            <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(460px,580px)] xl:items-start xl:gap-6 2xl:grid-cols-[minmax(0,1fr)_minmax(520px,620px)]">
                <div className="min-w-0">
                    <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-blue-100 sm:text-xs sm:tracking-[0.25em] lg:text-sm">
                        Gestión de cursos
                    </p>

                    <h2 className="mt-2 text-xl font-bold sm:text-2xl lg:text-3xl [@media(max-height:760px)]:text-2xl">
                        Cursos
                    </h2>
                    ...
                    <p className="mt-2 max-w-2xl text-xs leading-5 text-blue-50 sm:text-sm sm:leading-6 [@media(max-height:760px)]:text-xs [@media(max-height:760px)]:leading-5">
                        Administra, crea, edita y publica
                        cursos. También puedes asignar
                        docentes y organizar cada curso por
                ...
                <div className="grid w-full grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
                    {statItems.map((item) => (
                        ...
                        >
                            <p className="truncate text-[9px] font-bold uppercase tracking-wide text-white/75 sm:text-[10px] lg:text-xs">
                                {item.label}
                            </p>

                            <p className="mt-1 text-xl font-bold sm:mt-2 sm:text-2xl lg:text-3xl [@media(max-height:760px)]:text-2xl">
                                {isLoading
                                    ? "..."
                                    : stats[item.key]}
                            </p>
